...
2. Remove httplib's automatic Content-Length insertion when data is a
   file-like object.
"""
...
from http.client import _CS_REQ_STARTED, _CS_REQ_SENT, \
    CannotSendHeader, NotConnected
        ...
        if hasattr(data, 'read') and not isinstance(data, array):
            ...
            while datablock:
                ...
                if is_chunked:
                    ...
                    lenstr = hex(len(datablock))[2:].upper()
                    self.sock.sendall(('%s\r\n%s\r\n' %
                                       (lenstr, datablock)).encode())
                else:
