...
def PutFile(url, bucket, key, mimeType, fileBuffer, customMeta='', callbackParams=''):
    if mimeType == '':
        mimeType = 'application/octet-stream'
    entryURI = bucket + ':' + key
    action = '/rs-put/' + urlsafe_b64encode(entryURI) + '/mimeType/' + urlsafe_b64encode(mimeType)
    if customMeta != '':
        action += '/meta/' + urlsafe_b64encode(customMeta)
    params = {'file': fileBuffer, 'action': action}
    if callbackParams != '':
        if isinstance(callbackParams, dict):
            callbackParams = urllib.urlencode(callbackParams)
        params['params'] = callbackParams
...
def UploadFileByBuffer(bucket, key, mimeType, fileBuffer, customMeta='', callbackParams='', upToken=''):
    if mimeType == '':
        mimeType = 'application/octet-stream'
    entryURI = bucket + ':' + key
    action = '/rs-put/' + urlsafe_b64encode(entryURI) + '/mimeType/' + urlsafe_b64encode(mimeType)
    if customMeta != '':
        action += '/meta/' + urlsafe_b64encode(customMeta)
    params = {'action': action, 'file': fileBuffer}
    if callbackParams != '':
        if isinstance(callbackParams, dict):
            callbackParams = urllib.urlencode(callbackParams)
        params['params'] = callbackParams
