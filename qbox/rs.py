...
class Service:
    ...
    def Batch(self, act, keys):
        """
         * func Batch(action string, keys list) => Entries
         * 针对资源表的批量操作
         * action 可以是 get | stat | delete 等
        """
        oplist = ['op=/' + act + '/' + urlsafe_b64encode(self.TableName + ':' + k) for k in keys]
        opstr = '&'.join(oplist)
